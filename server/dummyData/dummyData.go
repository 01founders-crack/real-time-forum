...
func AddDummyData() {
	var num int
	err := database.MyDB.QueryRow("SELECT COUNT(*) FROM Users").Scan(&num)
	if err != nil {
		log.Fatal(":(", err)
	}
	if num == 0 {
		password, err := utils.HashPassword("1loveLemons")
		if err != nil {
			log.Fatal(";-;", err)
		}
		err = database.AddUser(models.User{Nickname: "LemonLover", Age: rand.Intn(100), Gender: []string{"Male", "Female"}[rand.Intn(2)], FirstName: "Lemon", LastName: "Lover", Email: "lemon@lover.com", Password: password})
		if err != nil {
			log.Fatal("o-O", err)
		}

		password, err = utils.HashPassword("1hateLemons")
		if err != nil {
			log.Fatal(";-;", err)
		}
		err = database.AddUser(models.User{Nickname: "LemonHater", Age: rand.Intn(100), Gender: []string{"Male", "Female"}[rand.Intn(2)], FirstName: "Lemon", LastName: "Hater", Email: "lemon@hater.com", Password: password})
		if err != nil {
			log.Fatal("o-O", err)
		}

		password, err = utils.HashPassword("eatCitricAc1d")
		if err != nil {
			log.Fatal(";-;", err)
		}
		err = database.AddUser(models.User{Nickname: "CitricAcidEater", Age: rand.Intn(100), Gender: []string{"Male", "Female"}[rand.Intn(2)], FirstName: "Citric", LastName: "Eater", Email: "citric.acid@eater.com", Password: password})
		if err != nil {
			log.Fatal("o-O", err)
		}

		password, err = utils.HashPassword("elevator")
		if err != nil {
			log.Fatal(";-;", err)
		}
		err = database.AddUser(models.User{Nickname: "EebyDeeby", Age: rand.Intn(100), Gender: []string{"Male", "Female"}[rand.Intn(2)], FirstName: "Eeby", LastName: "Deeby", Email: "eeby@deeby.com", Password: password})
		if err != nil {
			log.Fatal("o-O", err)
		}

		password, err = utils.HashPassword("pl1nk0H0r53")
		if err != nil {
			log.Fatal(";-;", err)
		}
		err = database.AddUser(models.User{Nickname: "HorsePlinko", Age: rand.Intn(100), Gender: []string{"Male", "Female"}[rand.Intn(2)], FirstName: "Horse", LastName: "Plinko", Email: " horse@plinko.com", Password: password})
		if err != nil {
			log.Fatal("o-O", err)
		}

		password, err = utils.HashPassword("BoatFace")
		if err != nil {
			log.Fatal(";-;", err)
		}
		err = database.AddUser(models.User{Nickname: "BoatyMcBoatface", Age: rand.Intn(100), Gender: []string{"Male", "Female"}[rand.Intn(2)], FirstName: "Boaty", LastName: "McBoatface", Email: " boaty@mcboatface.com", Password: password})
		if err != nil {
			log.Fatal("o-O", err)
		}
	}

	err = database.MyDB.QueryRow("SELECT COUNT(*) FROM Posts").Scan(&num)
	if err != nil {
		log.Fatal(":(", err)
	}
	if num == 0 {

		HorsePlinko, err := database.FindIdByNickname("HorsePlinko")
		if err != nil {
			log.Fatal("AAAA", err)
		}
		err = database.AddPost(models.Post{UserId: HorsePlinko, Title: "Facts about Bees", Content: `According to all known laws of aviation, there is no way a bee should be able to fly.
...
`, Category: "Category3"})
		...
		CitricAcidEater, err := database.FindIdByNickname("CitricAcidEater")
		if err != nil {
			log.Fatal("AAAA", err)
		}

		err = database.AddPost(models.Post{UserId: CitricAcidEater, Title: "Lemons", Content: "Lemons are DELICIOUS!", Category: "Category2"})
		if err != nil {
			log.Fatal("AAAA", err)
		}

		EebyDeeby, err := database.FindIdByNickname("EebyDeeby")
		if err != nil {
			log.Fatal("AAAA", err)
		}
		err = database.AddPost(models.Post{UserId: EebyDeeby, Title: "Colour Theory", Content: "Hospital", Category: "Category1"})
		if err != nil {
			log.Fatal("AAAA", err)
		}
		Boaty, err := database.FindIdByNickname("BoatyMcBoatface")
		if err != nil {
			log.Fatal("AAAA", err)
		}
		err = database.AddPost(models.Post{UserId: Boaty, Title: "Best Name", Content: "Gritsy Bitsy Teeny Weeny Yellow Anti-Slip Machiney", Category: "Category4"})
		if err != nil {
			log.Fatal("AAAA", err)
		}
	...
	err = database.MyDB.QueryRow("SELECT COUNT(*) FROM Comments").Scan(&num)
	if err != nil {
		log.Fatal(":(", err)
	}
	if num == 0 {
		LemonLover, err := database.FindIdByNickname("LemonLover")
		if err != nil {
			log.Fatal("AAAA", err)
		}
		LemonHater, err := database.FindIdByNickname("LemonHater")
		if err != nil {
			log.Fatal("AAAA", err)
		}
		LemonPost, err := database.FindPostsByTitle("Lemons")
		database.AddComment(models.Comment{PostId: LemonPost, UserId: LemonHater, Content: "0/10 Terrible Post"})
		database.AddComment(models.Comment{PostId: LemonPost, UserId: LemonLover, Content: "10/10 Great Post"})
	}

	err = database.MyDB.QueryRow("SELECT COUNT(*) FROM Messages").Scan(&num)
	if err != nil {
		log.Fatal(":(", err)
	}
	if num == 0 {
		...
		database.AddMessages(models.Message{SenderId: LemonLover, ReceiverId: EebyDeeby, Content: "Get drinked, idiot"})
		database.AddMessages(models.Message{SenderId: EebyDeeby, ReceiverId: LemonLover, Content: "lol"})
		for i := 1; i <= 20; i++ {
			senderIndex := rand.Intn(2)
			receiverIndex := 1 - senderIndex
			message := fmt.Sprintf("This is message number: %v", i)
			people := []uuid.UUID{LemonLover, EebyDeeby}
			database.AddMessages(models.Message{SenderId: people[senderIndex], ReceiverId: people[receiverIndex], Content: message})
		}
	}
